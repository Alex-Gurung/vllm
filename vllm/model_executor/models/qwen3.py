...
class Qwen3Model(Qwen2Model):
    ...
    def _init_reasoning_projector_from_base(self, last_mlp):
        """
        ...
        """
        if last_mlp is not None and self.reasoning_projector is not None:
            try:
        ...
        # LM head initialization will be handled during weight loading in Qwen3ForCausalLM
    ...
    def _apply_reasoning_steps(self, hidden_states: torch.Tensor, 
        ...
        """
        print(f"Applying reasoning steps")
        ...
        for step in range(self.reasoning_steps):
            print(f"Applying reasoning step {step+1} of {self.reasoning_steps}")
            # Get the last token's hidden state
            sample_context_hidden = last_hidden[:, -1, :]  # Shape: (batch_size, hidden_size)
            
            # Apply reasoning projector
            if self.reasoning_projector is not None:
                ...
                if self.reasoning_projector.vocab_projection:
                    ...
                    if hasattr(self, 'embed_tokens') and self.embed_tokens is not None:
                        ...
                        reasoning_projection = reasoning_projection @ self.embed_tokens.weight
        ...
        if hasattr(self, 'embed_tokens') and self.embed_tokens is not None:
            ...
            end_token_embeddings = self.embed_tokens(end_token_ids)
    ...
    def forward(
        ...
        if input_ids is not None and input_ids.shape[0] == 1:  # Only support batch size 1 for now
            print("Checking for reasoning triggers")
            print(f"reasoning trigger token in input_ids: {self.reasoning_trigger_token_id in input_ids}")
            ...
            if self.reasoning_trigger_token_id in input_ids[batch_idx]:
                ...
                for trigger_idx in trigger_indices:
                    ...
                    # Run normal forward on this section using parent class
                    section_hidden = super().forward(
                        section_input_ids, 
                        section_positions, 
                    ...
                    if current_hidden_states is None:
                        ...
                        # Handle both Tensor and IntermediateTensors cases
                        if isinstance(section_hidden, torch.Tensor) and isinstance(current_hidden_states, torch.Tensor):
                            current_hidden_states = torch.cat([current_hidden_states, section_hidden], dim=1)
                        else:
                            # If we get IntermediateTensors, we need to handle pipeline parallelism
                            # For now, just use the section_hidden as is
                            current_hidden_states = section_hidden
                    
                    # Apply reasoning steps to the current hidden states
                    if isinstance(current_hidden_states, torch.Tensor):
                ...
                if processed_tokens < input_ids.shape[1]:
                    ...
                    remaining_hidden = super().forward(
                        remaining_input_ids,
                        remaining_positions,
                        intermediate_tensors,
                    ...
                    if current_hidden_states is not None:
                        # Handle both Tensor and IntermediateTensors cases
                        if isinstance(remaining_hidden, torch.Tensor) and isinstance(current_hidden_states, torch.Tensor):
                            current_hidden_states = torch.cat([current_hidden_states, remaining_hidden], dim=1)
                        else:
                            # If we get IntermediateTensors, we need to handle pipeline parallelism
                            # For now, just use the remaining_hidden as is
        ...
        # No reasoning triggers found, run normal forward using parent class
        return super().forward(input_ids, positions, intermediate_tensors,
                               inputs_embeds, input_document_ids=input_document_ids)
