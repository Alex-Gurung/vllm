...
class GPUModelRunner(LoRAModelRunnerMixin, KVConnectorModelRunnerMixin):
    ...
    def _prepare_inputs(
        ...
        # REASONING EXTENSION: Check if any request will generate reasoning tokens
        # and pre-extend the scheduled token count to accommodate them.
        # This ensures attention metadata allocates proper sequence lengths.
        if (hasattr(self.model, "handles_full_sequence") and 
            hasattr(self.model, "get_num_reasoning_tokens") and 
            self.model.handles_full_sequence):
            
            print(f"=== REASONING TOKEN PREDICTION ===")
            for i, req_id in enumerate(req_ids):
                # Get the full token sequence for this request
                req_index = self.input_batch.req_id_to_index[req_id]
                num_tokens = self.seq_lens_cpu[req_index] 
                token_ids = self.input_batch.token_ids_cpu[req_index, :num_tokens]
                
                print(f"Request {req_id}: checking {num_tokens} tokens for reasoning patterns")
                
                # Ask the model how many reasoning tokens it would generate
                predicted_reasoning_tokens = self.model.get_num_reasoning_tokens(token_ids)
                
                if predicted_reasoning_tokens > 0:
                    print(f"Request {req_id}: predicted {predicted_reasoning_tokens} reasoning tokens")
                    print(f"Extending num_scheduled_tokens[{i}] from {num_scheduled_tokens[i]} to {num_scheduled_tokens[i] + predicted_reasoning_tokens}")
                    
                    # Extend the number of scheduled tokens to include predicted reasoning tokens
                    # This makes vLLM allocate proper sequence length and KV cache space
                    num_scheduled_tokens[i] += predicted_reasoning_tokens
                else:
                    print(f"Request {req_id}: no reasoning tokens predicted")
            print(f"=== END REASONING TOKEN PREDICTION ===")
        
        max_num_scheduled_tokens = max(num_scheduled_tokens)
        
        # Update total_num_scheduled_tokens to reflect any reasoning token extensions
        # This ensures all downstream calculations (positions, indices, etc.) are correct
        updated_total_num_scheduled_tokens = int(np.sum(num_scheduled_tokens))
        if updated_total_num_scheduled_tokens != total_num_scheduled_tokens:
            print(f"=== REASONING ADJUSTMENT ===")
            print(f"Updated total_num_scheduled_tokens from {total_num_scheduled_tokens} to {updated_total_num_scheduled_tokens}")
            total_num_scheduled_tokens = updated_total_num_scheduled_tokens
            print(f"=== END REASONING ADJUSTMENT ===")
    ...
    def execute_model(
        ...
        with set_forward_context(
            ...
            if hasattr(self.model, "handles_full_sequence") and self.model.handles_full_sequence:
                ...
                # Debug logging before model call
                print(f"=== GPU MODEL RUNNER DEBUG - BEFORE MODEL CALL ===")
                print(f"position_offset: {input_batch.position_offset}")
                print(f"positions: {positions}")
                print(f"modified positions: {positions + input_batch.position_offset}")
                print(f"seq_lens_cpu[0]: {self.seq_lens_cpu[0]}")
                print(f"seq_lens[0]: {self.seq_lens[0]}")
                print(f"num_reqs: {self.input_batch.num_reqs}")
                print(f"token_ids length: {len(token_ids)}")
                
                # Debug attention metadata 
                if isinstance(attn_metadata, dict):
                    sample_layer = list(attn_metadata.keys())[0]
                    sample_meta = attn_metadata[sample_layer]
                    print(f"Sample attention metadata ({sample_layer}):")
                    if hasattr(sample_meta, 'max_seq_len'):
                        print(f"  max_seq_len (with reasoning space): {sample_meta.max_seq_len}")
                    if hasattr(sample_meta, 'seq_lens'):
                        print(f"  seq_lens (with reasoning space): {sample_meta.seq_lens}")
                    if hasattr(sample_meta, 'num_actual_tokens'):
                        print(f"  num_actual_tokens: {sample_meta.num_actual_tokens}")
                ...
                # Debug logging after model call
                print(f"=== GPU MODEL RUNNER DEBUG - AFTER MODEL CALL ===")
                print(f"num_additional_tokens returned by model: {num_additional_tokens}")
                print(f"old position_offset: {input_batch.position_offset}")
                
                # IMPORTANT: The model returns the actual number of reasoning tokens generated.
                # We pre-extended sequence lengths based on prediction, but now we need to 
                # update position_offset based on what actually happened.
                input_batch.position_offset += num_additional_tokens
                
                print(f"new position_offset: {input_batch.position_offset}")
                print(f"seq_lens_cpu[0] (includes pre-allocated reasoning space): {self.seq_lens_cpu[0]}")
                print(f"seq_lens[0] (includes pre-allocated reasoning space): {self.seq_lens[0]}")
                
                # Note: seq_lens stays extended to provide space for reasoning tokens.
                # This is correct - we allocated extra space and the model used some/all of it.
                print(f"=== END GPU MODEL RUNNER DEBUG ===")
            else:
